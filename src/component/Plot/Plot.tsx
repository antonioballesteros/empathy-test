...
export const isAboveAverage = (d: PlotValue, average?: [Point, Point]): boolean => {
...
const setColor = (d: PlotValue, literals: [Literal, Literal], average?: [Point, Point]) => {
...
const addTooltip = (svg: d3.Selection<SVGGElement, unknown, null, undefined>) => {
  return svg
    .append('text')
    .attr('id', 'tooltip')
    .attr('class', 'tooltip')
...
const addCircles = (
  ...
  svg
    ...
    .attr('data-d', (d: PlotValue) => JSON.stringify(d))
...
const Plot = ({ data = [], average, literals = defaultLiteral }: PlotType) => {
  ...
  useEffect(() => {
    ...
    const xScale = d3
      .scaleLinear()
      .domain([
        Math.min(Math.max(d3.min(data, (d: any) => d.x) - 50, 0), minX),
        d3.max(data, (d: any) => d.x) + 50,
      ])
      .range([0, svgWidth]);
    ...
    const yScale = d3
      .scaleLinear()
      .domain([
        Math.max(d3.min(data, (d: any) => d.y) - 50, 0),
        Math.max(
          d3.max(data, (d: any) => d.y),
          averageYfromMaxX || 0,
        ) + 50,
      ])
